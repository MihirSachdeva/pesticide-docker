// auth
export const AUTH_START = "AUTH_START";
export const AUTH_SUCCESS = "AUTH_SUCCESS";
export const AUTH_FAIL = "AUTH_FAIL";
export const AUTH_LOGOUT = "AUTH_LOGOUT";

// theme, drawer, back button, bottom nav and header title
export const CHANGE_THEME = "CHANGE_THEME";
export const HEADER_TITLE = "HEADER_TITLE";
export const TOGGLE_DRAWER = "TOGGLE_DRAWER";
export const UPDATE_BACK_BUTTON = "UPDATE_BACK_BUTTON";
export const UPDATE_BOTTOM_NAV = "UPDATE_BOTTOM_NAV";

// fetching data from API
// projects
export const FETCH_PROJECTS_START = "FETCH_PROJECTS_START";
export const FETCH_PROJECTS_SUCCESS = "FETCH_PROJECTS_SUCCESS";
export const FETCH_PROJECTS_FAIL = "FETCH_PROJECTS_FAIL";

// issues
export const FETCH_ISSUES_START = "FETCH_ISSUES_START";
export const FETCH_ISSUES_SUCCESS = "FETCH_ISSUES_SUCCESS";
export const FETCH_ISSUES_FAIL = "FETCH_ISSUES_FAIL";

// snackbar
export const SHOW_SNACKBAR = "SHOW_SNACKBAR";
export const HIDE_SNACKBAR = "HIDE_SNACKBAR";

// sidepanel data
export const FETCH_SIDEPANEL_DATA_START = "FETCH_SIDEPANEL_DATA_START";
export const FETCH_SIDEPANEL_DATA_SUCCESS = "FETCH_SIDEPANEL_DATA_SUCCESS";
export const FETCH_SIDEPANEL_DATA_FAIL = "FETCH_SIDEPANEL_DATA_FAIL";
export const SET_SEARCH_SIDEPANEL_DATA = "SET_SEARCH_SIDEPANEL_DATA";
